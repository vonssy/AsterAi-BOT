...
- Register Here : [Aster AI](https://astpoint.asterai.xyz/?ref=7OQCL)
- Login With X
...
1. **Clone The Repositories:**
   ```bash
   git clone https://github.com/vonssy/AsterAi-BOT.git
   ```
   ```bash
   cd AsterAi-BOT
   ```
...
- **tokens.txt:** You will find the file `tokens.txt` inside the project directory. Make sure `tokens.txt` contains data that matches the format expected by the script. Here are examples of file formats:
  ```bash
    your_aster_ai_access_token_1
    your_aster_ai_access_token_2
  ```
